...
import { generateSteps } from '../../algorithms/sorting/selection-sort.js';
...
export default function SelectionSort() {
  ...
  const getBoxStyle = (i) => {
    ...
    const isNewMin = currentData.type === 'new-min' && currentData.index === i;
    const isSelecting = currentData.type === 'select' && currentData.index === i;
    ...
    if (isNewMin) return { bg: '#fef2f2', text: '#dc2626', border: '#fca5a5' };
    if (isComparing) return { bg: '#fef3c7', text: '#92400e', border: 'var(--amber)' };
    if (isSelecting) return { bg: '#ede9fe', text: '#7c3aed', border: '#a78bfa' };
  ...
  const getBarBg = (i) => {
    ...
    if (isNewMin) return '#fca5a5';
    if (isComparing) return 'var(--amber)';
    if (isSelecting) return '#a78bfa';
    return 'var(--purple)';
  };
